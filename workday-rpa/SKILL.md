...
- データは全てプレーンCSV/JSON（Excel・フロンティアAIで直接編集可能に保つ）。ヘッダー行必須。制御行（現行版の終了マーカー「e」のような）は**生成しない**。
- **読み込み側は表記ゆれに寛容にする**（2026-08 実データ移行で必要になった）: plan.csv はExcelで手編集する前提のため、日付・時刻の書式が変わりうる。`planio.normalizeDate/normalizeTime` で以下を受け付ける — 日付: `YYYY-MM-DD` / `YYYY/M/D` / `M/D/YYYY`（旧ツール形式）、時刻: `HH:mm` / `H:mm` / `HHmm`（旧ツール形式。`27`→`00:27`、`342`→`03:42` のように4桁ゼロ埋めして解釈）。解釈できない行は読み込み時に捨てるので、旧ツールの `e,e,e,...` 終了マーカーも自動的に落ちる。
- 個人データ（data/・config.json）はコミットしない（.gitignore 済み）。
- 日付の区切りは AM5:00（config: dateSplitHour）。深夜勤務は前日扱い。
- **⚠️ 深夜帯の時刻は単純な数値比較で並べてはいけない**（2026-07-25 実データで発覚）: plan.csv は時刻を HH:mm でしか持たないため、深夜勤務の日は `14:22-14:44(休憩)` → `00:04-00:27(終了)` のように「後のブロックほど数値が小さい」状態になる。単純に開始時刻で昇順ソートすると順序が逆転し、「途中ブロックなのに終了」「最終ブロックなのに休憩」と誤検出する。並び替えには `injectlib.effectiveMinutes()`（区切り時刻より前は+24時間する）を使うこと。
- 深夜帯を含む日は、Workday上のどの日付欄に入れるべきか解釈が分かれる（現行版は「その勤務日の欄」に入れる運用）。inject.js は該当日を確認ゲートで警告し、本人に目視確認を促す。
...
- **⚠️ Workdayのカレンダー再描画は遅い。「変化していない＝失敗」と即断しない**（2026-07-25 実機で発覚）: 週送りボタンのクリックは効いているのに、600msの固定待ちでは再描画が間に合わず「週の移動ができませんでした」と誤判定して全日失敗した。対策として、週ラベルのテキストが**実際に変わるまで**待ち（`waitForFunction`に前回のテキストを渡す。`''`を渡すと即座に真になり待ちにならないので注意）、変化しなければもう一度クリックして待ち直す。タイムアウトは20秒。
- **⚠️ OK保存の直後はカレンダーが作り直される。すぐ次の操作をしてはいけない**（2026-07-26 実機で発覚）: 同じ日の2ブロック目を入力しようとしたとき、`WASO` というdivがオーバーレイとしてクリックを横取りし（`intercepts pointer events`）、遮られている間に「時間を入力」要素自体がDOMから消えて `locator.click: Timeout 30000ms exceeded` で失敗した。対策は3点セット: (1) 保存後に前回の「時間を入力」プレースホルダが消えるまで待つ(`waitForCalendarSettled`)、(2) クリックが遮られたら要素へ直接 mousedown/mouseup/click を dispatch する(`clickRobust`)、(3) それでも駄目なら**グリッドをクリックする高さを変えて数回やり直す**（既存予定との重なり対策も兼ねる）。
- **dry-runでもポップアップは自動で閉じる**: 人間に手動で閉じさせるとEnterのタイミングとぶつかり画面が壊れる（実機で発生）。`キャンセル`→`閉じる`→Escapeの順に試す。
