...
- **Phase 3 (Injector): ✅ 実機検証完了・Go（2026-07-26）**
  - Shotaの実Workdayで2026-07の全11ブロックを入力完了。中断→再開で入力済み5件が正しくスキップされ、残り6件が入ることも実証済み
  - 実機で発見し修正したバグ3件（いずれもモックで実機同等の失敗を再現してから修正）:
    1. 深夜勤務の日でブロック順序が逆転し不正と誤検出（`effectiveMinutes`で解決）
    2. 週移動の待ち不足で「移動できない」と誤判定（ラベル変化を実際に待つ＋再クリック）
